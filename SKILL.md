...
当检测到用户需要生成实验报告时，**必须先通过 AskUserQuestion 工具**依次收集以下信息：

### 第一轮询问（必填信息）

| 问题 | 选项/说明 |
|------|----------|
| 编程语言 | Python / Java / C / C++ / 其他 |
| 代码风格 | 大学生水平（推荐）/ 最优实践 / 简洁版 |
| 实验日期 | 具体日期或让用户选择 |
...
| 问题 | 选项/说明 |
|------|----------|
| 实验内容来源 | 用户上传图片 / 用户直接提供代码 / AI根据主题生成 |
| 截图方式 | 浏览器渲染（推荐）/ 用户自己提供截图 |

> **注意**：如果用户在对话中已经提供了部分信息（如姓名、学号等），不要重复询问，直接使用已知信息。
