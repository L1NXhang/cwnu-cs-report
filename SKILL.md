...
> - 课程名称、实验名称、实验内容为**必填项**，缺一不可
> - 表头信息（年级、班级、姓名、学号、日期、指导老师）**可由用户选择**：AI 询问填写，或留空由用户手动在 Word 中填入
...
> **关于可选字段**：用户可以选择"跳过"或"留空"，之后在生成的 .docx 中手动填写。也可以选择由 AI 填入。
...
在编写配置 JSON 和填充报告之前，**必须**先将核心代码输出到对话中供用户审阅：

1. **输出代码**：将编写好的代码完整地展示在对话中
2. **等待确认**：明确告知用户"请确认代码内容，确认后我将生成完整的实验报告"
3. **收到确认后继续**：用户确认无误后，再继续编写配置 JSON 和执行 fill/pack

> **禁止**：跳过代码确认步骤直接生成报告。用户可能需要修改代码逻辑、变量名、注释风格等。
